  ...
  <div class="main">
    ...
    <form v-on:submit.prevent="addNewReview" v-show="showForm">
      <div class="form-element">
        <label for="reviewer" v-bind:class="{ error: reviewerInvalid }">
          Name:</label
        >
        <input
          type="text"
          id="reviewer"
          v-model.trim.lazy="newReview.reviewer"
        />
        <div class="error" v-if="reviewerInvalid">Name is required</div>
      </div>
      <div class="form-element">
...
export default {
  ...
  methods: {
    addNewReview() {
      if (
        this.newReview.reviewer === undefined ||
        this.newReview.reviewer.length === 0
      ) {
