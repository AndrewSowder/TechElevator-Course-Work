...
FROM city 
WHERE area < 25 AND popluation > 100000;
...
FROM city
WHERE city.state_abbreviation = 'CA' OR city.state_abbreviation = 'FL';
...
SELECT city.city_name, city.population
FROM city
WHERE city.state_abbreviation IN ('CT', 'ME', 'MA','NH','RI','VT');
...
SELECT park_name, area * 1000 * 1000 AS area_in_meters
FROM park;
...
SELECT park_name, date_established
FROM park
WHERE date_established < '19000101';

SELECT park_name, date_established
FROM park
WHERE date_established BETWEEN '18000101' AND '1/1/1900';
